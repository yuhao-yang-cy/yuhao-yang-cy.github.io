...
$$A_1, A_2$$ 为任意常数，并且可以取复数。

取 $$\tilde{x}_c(t)$$ 实部，并重新定义一系列新的常数，可以得到

$$ \boxed{x_c(t) = x_0\mathrm{e}^{-\beta t} \cos(\omega't + \phi)} $$

$$x_0$$ 和 $$\phi$$ 的取值都将由振动的初始条件决定。但是注意到式子中的 $$\mathrm{e}^{-\beta t}$$ 项，这告诉我们 $$x_c(t)$$ 对总位移的贡献会随时间指数衰减。在充分长的时间后，这一项的贡献就可以忽略不计。
...
$$ \displaystyle \tilde{x}_p(t) = \frac{f}{-\omega^2 + 2\beta \omega i + \omega_0^2}\mathrm{e}^{i\omega t} $$

为了提取实部，我们先对分母有理化：

$$ \displaystyle \tilde{x}_p(t) = \frac{f(\omega_0^2-\omega^2-2\beta\omega i)}{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}\mathrm{e}^{i\omega t} $$

这一大坨东西的实部为：

$$\displaystyle x_p(t) = \frac{f}{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}\left[ (\omega_0^2-\omega^2)\cos\omega t + 2\beta \omega \sin\omega t \right]$$
...
$$ \displaystyle \boxed{ x_p(t) = \frac{f}{\sqrt{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}} \cos(\omega t - \psi) }$$
...
$$ \begin{aligned}(\omega_0^2 - \omega^2)^2 + 4\beta^2\omega^2 &= \omega^4 - 2(\omega_0^2 - 2\beta^2) \omega^2 + \omega_0^4 \\ &= \Big[ \omega^2 - (\omega_0^2 - 2\beta^2) \Big]^2 + \omega_0^4 - (\omega_0^2 - 2\beta^2)^2 \\ & = \Big[ \omega^2 - (\omega_0^2 - 2\beta^2) \Big]^2 + 4\beta^2(\omega_0^2-\beta^2)\end{aligned}$$
...
$$ \displaystyle \cos\psi = \frac{\omega_0^2-\omega^2}{\sqrt{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}}$$

$$ \displaystyle \sin \psi = \frac{2\beta\omega}{\sqrt{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}}$$

合并以上两式可以得到

$$ \displaystyle \boxed{ \tan\psi = \frac{2\beta\omega}{\omega_0^2 - \omega^2} }$$
